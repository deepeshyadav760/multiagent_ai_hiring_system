...
// /**
//  * Switch to a new tab and load its data.
//  * @param {string} tabName - The name of the tab to switch to.
//  */
// function switchTab(tabName) {
//     if (tabName === currentTab) return; // Do nothing if already on the same tab
//     currentTab = tabName;
...
// /**
//  * Load data from the API based on the current active tab.
//  */
...
//         switch (currentTab) {
//             case 'dashboard':
//                 // The API likely returns stats in a nested object, e.g., { "success": true, "stats": {...} }
//                 data = await fetchData('/stats');
//                 // Use nested 'stats' property if it exists, otherwise use the root object
//                 renderDashboard(data.stats || data);
//                 break;
//             case 'jobs':
//                 data = await fetchData('/jobs/');
//                 renderJobs(data.jobs || []); // Expects an array under the "jobs" key
//                 break;
//             case 'candidates':
//                 data = await fetchData('/candidates/');
//                 renderCandidates(data.candidates || []); // Expects an array under the "candidates" key
//                 break;
//             case 'interviews':
//                 data = await fetchData('/interviews/');
//                 renderInterviews(data.interviews || []); // Expects an array under the "interviews" key
//                 break;
//         }
//     } catch (error) {
//         showNotification(`Error loading data: ${error.message}`, 'error');
//         // Clear containers on error to avoid showing stale data
//         document.getElementById('jobs-container').innerHTML = createEmptyState('Could not load data.', 'Please check the server connection and try again.');
//         document.getElementById('candidates-container').innerHTML = createEmptyState('Could not load data.', 'Please check the server connection and try again.');
//         document.getElementById('interviews-container').innerHTML = createEmptyState('Could not load data.', 'Please check the server connection and try again.');
//     } finally {
...
// /**
//  * Handle the resume file upload process.
...
//     // Your backend might expect the key 'file' for FastAPI's UploadFile
//     formData.append('file', file);

//     try {
//         // Use the correct endpoint name for resume upload, e.g., /upload-resume/
...
//             throw new Error(result.detail || 'Upload failed due to a server error.');
//         }

//         showNotification(result.message || 'Resume processed successfully!', 'success');
        
//         // Refresh data to show the new candidate
//         if (currentTab === 'candidates' || currentTab === 'dashboard') {
//             await loadData();
//         } else {
//             // If on another tab, switch to the candidates tab to show the result
...
//  * @param {string} endpoint - The API endpoint to fetch from (e.g., '/stats').
...
//                     <p class="card-subtitle">${job.location || 'N/A'} | ${job.type || 'N/A'}</p>
//                 </div>
//                 <span class="badge ${job.status === 'Open' ? 'badge-green' : 'badge-gray'}">${job.status || 'N/A'}</span>
//             </div>
...
//             </div>
//             <div class="skills-tags">
//                 ${(candidate.skills || []).map(skill => `<span class="skill-tag-purple">${skill}</span>`).join('')}
//             </div>
//         </div>
//     `).join('');
// }
...
//             <div class="card-header">
//                 <div>
//                     <h3 class="card-title">Candidate ID: ${interview.candidate_id}</h3>
//                     <p class="card-subtitle">For Job ID: ${interview.job_id}</p>
//                 </div>
//                 <span class="badge ${interview.status === 'scheduled' ? 'badge-blue' : 'badge-green'}">${interview.status}</span>
//             </div>
//             <div class="card-footer">
//                 <span>Date: ${new Date(interview.scheduled_time).toLocaleString()}</span>
//             </div>
...
// /**
//  * Creates an HTML string for an empty state message.
//  * @param {string} title - The main message.
//  * @param {string} subtitle - The secondary, smaller message.
